<?php
    ...
    if ($_POST['action'] == "create") { //check if the action type of create has been set in the post request
        ...
        //insert gift item
        $guest = $db->prepare('INSERT INTO gift_list (gift_item_name, gift_item_desc, gift_item_url, gift_item_type) VALUES (?,?,?,?)');
        $guest->bind_param('ssss',$gift_item_name, $gift_item_desc, $gift_item_url, $gift_item_type);
        $guest->execute();
        $guest->close();
